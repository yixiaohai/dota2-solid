...
class ExcelParser {
        ...
        headers.forEach((header, index) => {
            ...
            if (trimmed.endsWith(CONFIG.BLOCK_MARKERS.START)) {
                const newBlock: BlockInfo = {
                    name: trimmed.slice(0, -1).trim(), // 确保移除末尾的{
                    start: index,
                    end: -1,
    ...
    private processBlock(
        ...
        if (hasContent) {
            entries.push(
                `${indent}"${this.escape(block.name)}" {`, // 使用处理后的名称
                ...allEntries,
                `${indent}}`
            );
        }
    ...
    parseSheet(data: unknown[][]): { kvContent: string | null; langData: LangData } {
...
class KVConverter {
    ...
    constructor() {
        this.setupWatcher();
        console.log(`[${color.magenta('excel.ts')}] 👁️  监听目录: ${CONFIG.EXCEL_DIR}`);
    }
    ...
    private async processFile(filePath: string) {
        try {
            console.log(`[${color.magenta('excel.ts')}] 🔄 处理文件: ${path.basename(filePath)}`);
            ...
            workbook.SheetNames.filter(
                ...
                // 保存KV文件
                if (kvContent) {
					console.log(`[${color.magenta('excel.ts')}] 📝 生成kv: ${sheetName}.txt`);
    ...
    private saveLangFiles() {
        this.langData.forEach((entries, lang) => {
            ...
            console.log(`[${color.magenta('excel.ts')}] 🌐 生成语言文件: addon_${lang}.txt`);
        });
    }
}
